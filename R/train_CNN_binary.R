...
train_CNN_binary <- function(input.data.path, test.data, architecture,
  ...
  for(a in 1:length(epoch.iterations )){
    ...
    if( TrainingLabelsMatch == FALSE) {
      print( 'Training classes do not match! This is based on the order of the classes in the training folder')
    ...
    if(save.model==TRUE){
    luz_save(BinaryModel, paste( output.data.path,trainingfolder,n.epoch,architecture, "model.pt",sep='_'))
    }
    ...
    filename <- paste(output.data.path,'performance_tables/', trainingfolder, '_', n.epoch, '_', architecture, '_CNNDF.csv', sep = '')
