...
#'
#' @param input.data.path The path to the input data folder.
#' @param test.data.path The path to the test data folder.
#' @param unfreeze Logical. If TRUE, unfreeze all the layers of the pretrained AlexNet for retraining.
#'                 If FALSE, retain the weights of the pretrained layers. Default is TRUE.
...
#' @param trainingfolder.short Character. A shortened descriptor of the training data,
#'                             used for naming output files. Default is 'imagesmalaysiaHQ'.
#'
...
#' @examples
#' \dontrun{
#'   train_alexnet("path_to_input_data", "path_to_test_data")
#' }
#'
#' @seealso [torch::nn_module()] and other torch functions.
#'
#' @export
...
train_alexnet <- function(input.data.path, test.data.path, unfreeze = TRUE,
                          epoch.iterations, early.stop = 'yes',
                          output.base.path = 'data/',
                          trainingfolder.short) {
  ...
  output.data.path <- paste(output.base.path,'output','unfrozen', unfreeze, trainingfolder.short, '/', sep='_')
...
for(a in 1:length(epoch.iterations )){
  ...
  n.epoch <- epoch.iterations [a]
    ...
    modelAlexNetGibbon <- fitted %>%
      fit(train_dl, epochs = n.epoch, valid_data = valid_dl,
          callbacks = list(
            ...
            luz_callback_model_checkpoint(path = paste(output.data.path, trainingfolder.short,'/', sep = '_')),
            luz_callback_csv_logger(paste(output.data.path, trainingfolder.short, n.epoch, "logs_AlexNet.csv", sep = '_'))
          ),
          verbose = TRUE
      )
    ...
    TempCSV.AlexNet <- read.csv(paste(output.data.path, trainingfolder.short, n.epoch, "logs_AlexNet.csv", sep = '_'))
