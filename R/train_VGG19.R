...
#'
#' This function is designed to train the VGG19 model on a given dataset.
...
#' @param unfreeze Logical. Determines if all layers of the pretrained VGG19 should be unfrozen for retraining.
#'                 Default is TRUE.
...
train_VGG19 <- function(input.data.path, test.data, unfreeze = TRUE,
  ...
  for(a in 1:length(epoch.iterations )){
    ...
    VGG19Class <- ifelse((VGG19Prob) < 0.5, positive.class, negative.class)
    ...
    for (threshold in thresholds) {
      # VGG19
      VGG19PredictedClass <- ifelse((outputTableVGG19$Probability) < threshold, positive.class, negative.class)
