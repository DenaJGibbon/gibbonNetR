...
2. **Ensure Consistent File Formats:**  
   ...
   - Make sure that each folder contains only .wav files corresponding to the correct label. Files should not be mixed across labels. Incorrect labeling can lead to poor model performance.
...
The "spectrogram_images" function below puts 100% of the test images into the test folder. This dataset contains two classes: female northern grey gibbons, and a noise category.
```{r eval=FALSE}
library(gibbonNetR)
